...
`$wrap-up`은 merge 후 정리 스킬이므로, 가능한 경우 실제 merge target을 사용합니다. 레포 이름만으로 target을 강제하지 않습니다.
...
1. Linear 이슈를 재조회합니다.
2. AC 항목별 증빙 텍스트를 파싱합니다.
3. 아래 기준으로 미흡 여부를 판정합니다.
4. 미흡 항목을 재수집하고 evidence workflow로 description을 업데이트합니다.
...
1. 실패 항목에서 미흡 유형을 파악합니다.
2. Section 3의 보강 방법에 따라 증빙을 재수집합니다.
3. evidence workflow로 description을 업데이트합니다.
4. validator workflow를 재실행합니다.

### 수동 업로드 후 매핑 프로세스

1. 사용자가 Linear 이슈에 스크린샷/동영상을 업로드한 뒤 알립니다.
2. Linear 이슈를 재조회합니다.
3. 업로드된 이미지를 확인합니다.
4. 각 이미지 내용을 열람하여 AC 항목과 매칭합니다:
   - 이미지 내용 (UI 화면, 로그, 터미널 등) 분석
   - AC 항목의 요구사항과 비교
   - 가장 적절한 AC 항목에 매핑
5. description에 증빙 텍스트를 업데이트합니다:

       - [x] 동작 확인 → 결과물: 스크린샷 4장
             ↳ Kafka 로그, 감사 이력 목록, 감사 이력 상세, 채팅 UI

6. 매핑 결과를 사용자에게 보여주고 다음 단계로 진행합니다.
