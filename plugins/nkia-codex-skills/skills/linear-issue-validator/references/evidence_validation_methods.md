...
✅ 올바른 검증:
   1. 페이지 내용 확인: "플러그인 만들기 - Codex Code Docs"
   2. 메타 설명: "슬래시 명령어, 에이전트, 훅, 스킬 및 MCP 서버를 사용하여 Codex Code를 확장..."
...
이미지를 성공적으로 열람한 후:
1. **이미지 내용 분석**: Codex의 vision 기능으로 이미지에 무엇이 표시되어 있는지 파악
...
**검증 기준:**
- 이미지/동영상이 **실제로 접근 가능**한가? (이미지 보기 도구 또는 HTTP 접근)
