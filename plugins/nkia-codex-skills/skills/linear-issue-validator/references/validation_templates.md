...
> # ⚠️⚠️⚠️ MUST: `<details><summary>` 태그로 감싸서 작성 ⚠️⚠️⚠️
>
> **MR Diff 섹션은 반드시 아래 HTML 형식으로 작성합니다. 평문 마크다운으로 펼쳐 쓰면 코멘트가 길어져서 가독성이 망가집니다.**
>
> - 최상위 `## 📝 MR Diff 분석` 헤더 직후, 각 MR마다 `<details>` 블록 1개씩
> - `<summary>` 안에는 MR 식별자 + 시스템명 + 상태 아이콘만 (한 줄)
> - 디렉토리 트리 + AC 커버리지는 모두 `<details>` 안에 넣어 기본 접힘 상태로 둠
> - **이 규칙은 매 검증마다 반드시 지켜야 합니다. 평문으로 펼쳐서 작성하면 안 됩니다.**
>
> 과거 여러 번 평문으로 작성된 사례가 있어 강조합니다 — `<details>`/`<summary>`를 빠뜨리면 검증 결과 코멘트가 무효 처리됩니다.
...
<details>
<summary>{{mr_identifier}} ({{system_name}}) — {{mr_state_icon}} {{mr_state}}</summary>
...
> ⚠️ 위 코드 블록의 `<details>` / `<summary>` 태그는 **장식이 아니라 필수 형식**입니다. MR이 여러 개면 `<details>` 블록을 MR마다 반복합니다.

**예시 1 — case: 압축 불필요 (분기점이 가까워 압축할 단일 자식 체인 없음):**
```markdown
<details>
<summary>PR #1: nkia-ai-team/claude-code-skills#1 — ✅ merged</summary>
...
```markdown
<details>
<summary>MR 3: lucida-chat-ai!789 (Chat AI) — ✅ merged</summary>
...
### 5.1 검증 완료 — 상태 유지 안내 (기본)

**기본 동작:** 검증이 통과해도 자동 전환하지 않습니다. 코멘트 본문에 아래 안내만 포함하고 종료합니다. 사용자에게 묻지 않습니다.
...
### 현재 상태
- 이슈 상태: {{current_state}} (유지)
- AC: ✅ {{ac_total}}/{{ac_total}} 완료

### 다음 단계 (사용자가 직접 처리)

validator는 상태를 자동 전환하지 않습니다. 필요하면 직접 In Review 또는 Done으로 옮겨주세요.
```

### 5.1b 명시적 전환 요청이 있는 경우에만

사용자가 호출 시 "검증 통과하면 In Review/Done으로 옮겨줘" 같이 **명시적으로** 요청한 경우에만 `mcp__linear__save_issue`로 사용자가 지정한 상태로 전환합니다. 명시 요청이 없으면 묻지 말고 현재 상태를 유지합니다.
...
image:
  ...
  validation_rule: |
    ...
    2. 이미지 보기 도구로 이미지 내용을 시각적으로 확인 (vision)
